...
#ifndef IMAGE_RANDOMCLOCK_ARBITRARY_H
#define IMAGE_RANDOMCLOCK_ARBITRARY_H
#include "read_data.h"
#include "map"
...
//default_random_engine e2(1);
Result RandomClockArbitrary(double eps,double B)
{
    cout<<"RCA & Budget: "<<B<<"---------start---------"<<endl;

    //random
    double p=sqrt(5.0)/(3.0+sqrt(5.0));
...
//            continue;
    ...
    while(true)
    {
        ...
        //initialize C and A
        list<int> C;//mark all nodes are available
        //bool empty=true;
...
//        cout<<"now gamma: "<<gamma<<endl;
        ...
        while (true) {
            ...
            if (C.empty())//non element or marginal gain<=0
            {
                C_empty=true;
                break;
            }

            uniform_int_distribution<unsigned> arbitrary(0,C.size()-1);
            int place=arbitrary(e2);
            int counter=0;

            list<int>::iterator element;
            for(auto e=C.begin();e!=C.end();)//remove unfeasible element
            {
                if(counter==place)
                {
                    element=e;
            ...
            double marginal=S[t%2].marginal(*element);
            oracle_times++;

            //S<-S\cup {u}
            l_u[*element]=min(l_u[*element],marginal*B/gamma);

            double price=l_u[*element];
            bool accept=false;

            //cout<<"price: "<<price<<endl;

            if(price>=contrast_cost[*element])
            {
                if(S[t%2].S_revenue+marginal>gamma)
                    break;
                if(u(e2)==1) {
                    S[t % 2].add_element_truth(marginal, *element, price);
                    accept=true;
                }
            }
            if(!accept)
                D[*element]=0;
            element=C.erase(element);//discarded or selected
        }
//        cout<<"t: "<<t<<endl;
//        for(const auto& it:S) {
//            cout<<"S: "<<endl;
...
//            int counter=0;
            ...
            list<int>::iterator element=C.begin();
//            for(auto e=C.begin();e!=C.end();)//remove unfeasible element
//            {
//                if(counter==place)
//                {
...
//            }
    ...
    cout<<"RandomClock ---------end--------- "<<endl<<endl;
    return Result(S_star.S_revenue,S_star.S_cost,S_star.Set.size(),oracle_times);

}
#endif //IMAGE_RANDOMCLOCK_ARBITRARY_H
