...
#ifndef IMAGE_RANDOMCLOCK_H
#define IMAGE_RANDOMCLOCK_H
#include "read_data.h"
#include "map"
class Ai_node
{
...
//            break;
        ...
        while (true) {
            ...
            pair<int, double> temp = Boost(A, S[t%2], C, B);
...
/*            for (int iter = 0; iter < node_num; iter++) {
                ...
                double temp_value=S[t%2].marginal(iter);
                if(temp_value<=max_marginal&&temp_value>=0)
                {
                    max_marginal=temp_value;
                    max_element=iter;
                }
            }*/
            //test
